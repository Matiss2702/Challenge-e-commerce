...
<template>
        <section
                class="min-h-screen-nhm-mobile lg:min-h-screen-nhm-desktop container my-12 lg:my-16 flex-1 flex flex-col gap-12">
                ...
                <div class="flex-1 sm:grid gap-6"
                        ...
                        <aside v-if="fullProductSheetCount !== 0">
                                <div class="sticky top-28 flex flex-col items-center">
                                        <TheFilters :filters="computedFilters" v-model:filters-value="filtersValue" />

                                        <span class="pt-4 opacity-50">{{ $pt('quantityProduct', {
                                                count:
                                                fullProductSheetCount}) }}</span>
                                </div>
                        </aside>

                        <div v-if="productSheets && productSheets.length">
                                <ProductPagination v-if="currentPage > 1" :total="fullProductSheetCount"
                                        :current-page="currentPage" :product-per-page="40"
                                        @update="page => currentPage = page" :key="'top-pagination-' + currentPage" />

                                <div class="grid grid-cols-1 gap-6 sm:grid-cols-2 md:grid-cols-3 lg:grid-cols-4">
                                        <ProductCard v-for="(product, index) in productSheets" :key="index"
                                                :product="product" class="w-full mx-auto max-w-80" />
                                </div>

                                <ProductPagination v-if="fullProductSheetCount >= 40" :total="fullProductSheetCount"
                                        :current-page="currentPage" :product-per-page="40"
                                        @update="page => currentPage = page"
                                        :key="'bottom-pagination-' + currentPage" />
                        </div>

                        <div v-else class="flex flex-col items-center justify-center h-full gap-1 text-center">
                                ...
                                <PrimaryButton class="mt-4" as-child>
